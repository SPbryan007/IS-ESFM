    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <el-table
          stripe
          :data="pageOfItems"
          :default-sort="{prop: 'fecha_usuario', order: 'descending'}"
          style="width: 100%"
        >
          <el-table-column type="index" width="50"></el-table-column>
          <el-table-column prop="nombre" label="Nombre" width="130" sortable></el-table-column>
          <el-table-column prop="apellido" label="Apellido" width="170" sortable></el-table-column>
          <el-table-column label="Estado" width="130" sortable>
            <template slot-scope="scope">
              <el-tag
                :type="scope.row.deleted_at? 'success':'danger'"
                effect="dark"
              >{{ scope.row.deleted_at? 'ACTIVO':'INACTIVO' }}</el-tag>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ViewDetails(id) {
      store.dispatch("usuario/getDetalleById", id);
    },
    ...mapMutations("usuario", ["SET_EDIT_FORM", "CLEAR_FORM"]),
    OnClickDelete(index, row) {
      this.$confirm(
        `¿ Esta seguro que desea eliminar el usuario ${row.nombre} ?`,
        "Advertencia",
        {
