<template>
    <div class="">
        ...
        <el-alert
            ...
            effect="dark">
            <template slot="title">
                <span class="font-weight-bold" style="font-size: 15px"> Reportes de Ingresos</span>
            </template>
        </el-alert>
...
<!--                    </el-form-item>-->
        ...
        <table class="table table-bordered text-center" style="background-color: white; font-size:12px">
            ...
            <tr>
                ...
                <th scope="col" class="pa">Nro Ingreso</th>
                <th scope="col" class="pa">Fecha</th>
                <th scope="col" class="pa">Proveedor</th>
                <th scope="col" class="pa">Formulario</th>
                <th scope="col" class="pa">Nro Formulario/Acta</th>
                <th scope="col" class="pa">Total</th>
                <th scope="col" class="pa">Usuario</th>
            ...
            <tr v-for="(item, index) in pageOfItems"  :key="index">
                ...
                <td><b>NIA</b>{{ item.nro_ingreso  }}</td>
                <td>{{ item.created_at | dateformat }}</td>
                <td> {{ item.proveedor?  item.proveedor.nombre : '-' }}</td>
                <td>{{ getFormulario(item) }}</td>
                <td>{{ getNroFormulario(item) }}</td>
                <td class="text-left"> <b>BS.</b> {{ (item.total).toFixed(2) }}</td>
                <td>{{ item.usuario.funcionario.nombre+' '+item.usuario.funcionario.apellido }}</td>
                <td>
                    <router-link
                        v-on:click.native="$store.dispatch('ingreso/getDetalleById',item.id)"
                        :to="{ name: 'reporteviewdetailingreso',params:{id:item.id } }"
                    >
                        <el-button
                            size="mini"
                            type="warning"
        ...
        GET_ITEMS(form) {
            this.$refs[form].validate(valid => {
                ...
                if (valid) {
                    ...
                    axios.post('/controller/ingreso/query',this.consulta)
                        .then((response) =>{
