    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        >
          ...
          <el-table-column prop="responsable" label="Responsable" width="300" sortable>
            <template slot-scope="scope">
              <div>{{ `${scope.row.nombre_fun} ${scope.row.apellido_fun}` }}</div>
            </template>
          </el-table-column>
          <el-table-column label="Operacion">
            <template slot-scope="scope">
              ...
              <router-link
                ...
                :to="{ name: 'editunidad',params:{id:scope.row.idUnidad } }"
              >
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("unidad", ["GET_FILTER_ITEMS"]),
    PerPage: function () {
  ...
  methods: {
    ...
    OnClickDelete(index, row) {
      this.$confirm(
        `¿ Esta seguro que desea eliminar la unidad ${row.nombre} ?`,
        "Advertencia",
        {
