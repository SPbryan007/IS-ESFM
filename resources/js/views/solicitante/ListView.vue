                    ...
                    >
                        <template slot-scope="scope">
                            <el-tag type="warning" disable-transitions>
                                <strong>{{scope.row.unidad.nombre}}</strong>
                            </el-tag>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="Operacion" >
                        <template slot-scope="scope">
                            ...
                            <el-button
                                ...
                                @click="OnClickAD(scope.$index, scope.row)"
                            >Activar </el-button>
                            <!--<el-tooltip :content="`Estado: ${ scope.row.deleted_at ? 'Activo' : 'Inactivo'}`" placement="left">
                                <el-switch
                                    @change="OnChangeStatus(scope.$index, scope.row)"
                                    v-model="items[scope.$index].deleted_at"
                                    active-color="#13ce66"
...
export default {
    ...
    methods: {
        ...
        OnClickAD(index, row) {
            this.$confirm(
                `¿ Esta seguro que desea ${row.deleted_at? 'activar' : 'eliminar'} al solicitante ${row.funcionario.nombre} ${row.funcionario.apellido} ?`,
                "Advertencia",
                {
