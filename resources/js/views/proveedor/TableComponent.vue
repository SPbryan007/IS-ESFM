      ...
      <div class="row">
        <table class="table table-bordered" v-columns-resizable>
          ...
          <tr v-for="(item, index) in pageOfItems" :key="index">
            <td style="height:20px">{{ item.nombre }}</td>
            <td>{{ item.nit }}</td>
            ...
            <td>
              <button
                @click="OnClickEdit(item.idProveedor)"
                type="button"
              ...
              <button
                @click="OnClickDelete({id:item.idProveedor,progress:$Progress})"
                type="button"
        ...
        <center class="container">
          ...
          <div v-if="dt_no_found">
            <span class="text-info text-center">No se encontraron datos..</span>
          </div>
        </center>
      </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("proveedor", ["GET_FILTER_ITEMS_BY_DEFAULT"])
  },
  methods: {
    refresh: function() {
      let self = this.$refs;
    ...
    ...mapActions("proveedor", ["OnClickEdit", "OnClickDelete"]),
