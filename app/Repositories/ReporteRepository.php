...
//date('Y-m-d H:i:s',strtotime($data->del))
    ...
    public function SaldosAlmacen($del, $al, $periodo)
    {
        return Lote::select(
            ...
            DB::raw("(SELECT SUM(IFNULL(ds.cantidad,0))
                                FROM lote as l
                                WHERE `l`.`precio_u` <> 0 AND `l`.`articulo_id` = `a`.`id` AND `s`.`deleted_at` = NULL AND  `i`.`created_at`` <=".DB::getPdo()->quote(date('Y-m-d H:i:s', strtotime($del)))."

                                GROUP BY `a`.`id`, `l`.`precio_u`)
            as c_salida"),

            DB::raw('SUM(IFNULL(ds.cantidad,0)) as c_salidasa,SUM(IFNULL(ds.cantidad,0)*IFNULL(lote.precio_u,0)) as s_salida'),
